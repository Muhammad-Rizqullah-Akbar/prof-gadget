...
// 2. POST: Input Servis Baru
export const createService = async (req: AuthRequest, res: Response) => {
  try {
    const { customerName, customerPhone, customerAddress, deviceType, deviceModel, complaint } = req.body;
    const technicianId = req.user.id; // Ambil ID teknisi dari Token Login

    // Generate Nomor Tiket Otomatis (Format: SRV-Timestamp)
    const ticketNumber = `SRV-${Date.now()}`;

    // Simpan ke Database (Pakai connectOrCreate untuk Customer)
    const newService = await prisma.service.create({
      data: {
        ...
        complaint,
        status: 'PENDING',
        technician: { connect: { id: technicianId } }, // Sambungkan ke Teknisi yang login
        customer: {
          connectOrCreate: {
            where: { phone: customerPhone }, // Cek jika no HP sudah ada
            create: { name: customerName, phone: customerPhone, address: customerAddress } // Jika belum, buat baru
          }
        }
      }
    });
